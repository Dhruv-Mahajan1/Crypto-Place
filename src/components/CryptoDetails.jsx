...
const CryptoDetails = () => {
  ...
  const { data, isFetching1 } = useGetCryptoDetailsQuery(coinId);
  const { data: coinHistory, isFetching2 } = useGetCryptoHistoryQuery({
    coinId,
    timePeriod,
  });
  const cryptoDetails = data?.data?.coin;
  const time = ["3h", "24h", "7d", "30d", "1y", "3m", "3y", "5y"];
  if (isFetching1) return <Loader />;
  if (isFetching2) return <Loader />;
  const stats = [
    {
      title: "Price to USD",
